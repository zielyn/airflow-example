<<<<<<< HEAD:st/amazon-mwaa-docker-images/tests/images/airflow/3.2.0/python/mwaa/test_entrypoint.py
# test_entrypoint.py
import pytest
import os
import sys
...
def test_mark_as_unhealthy_new_container(mock_environ):
    """Test _mark_as_unhealthy when container is new (within grace period)"""
    with patch('os.makedirs') as mock_makedirs, \
         patch('builtins.open', mock_open()) as mock_file, \
         patch('time.sleep') as mock_sleep, \
         patch('time.time', return_value=entrypoint.CONTAINER_START_TIME + 10):
        ...
        # Verify sleep was called (container is within grace period)
        mock_sleep.assert_called_once_with(1100)


def test_mark_as_unhealthy_old_container(mock_environ):
    """Test _mark_as_unhealthy when container has been running past grace period"""
    with patch('os.makedirs') as mock_makedirs, \
         patch('builtins.open', mock_open()) as mock_file, \
         patch('time.sleep') as mock_sleep, \
         patch('time.time', return_value=entrypoint.CONTAINER_START_TIME + 2000):
        ...
        # Verify sleep was NOT called (container past grace period)
...
def test_fix_shared_log_volume_permissions_dag_processor_dir_failure(mock_environ):
    ...
    with patch('os.path.ismount', return_value=True), \
        ...
        # Should not raise — just logs a warning
        entrypoint._fix_shared_log_volume_permissions()
>>>>>>> 72a97b2967a22ec1bd207f3e68db75e52ab1848f:st/amazon-mwaa-docker-images/tests/images/airflow/3.2.1/python/mwaa/test_entrypoint.py
